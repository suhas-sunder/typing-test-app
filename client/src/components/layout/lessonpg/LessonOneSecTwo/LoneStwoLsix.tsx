export default function LoneStwoLsix() {
  return (
    <div className="mx-auto max-w-4xl px-4 py-8">
      <h2 className="mb-4 text-3xl font-bold">
        Welcome to Your First Typing Lesson!
      </h2>
      <p>
        Today, we're going to embark on an exciting journey into the world of
        keyboards and typing. Even if you've never touched a keyboard before,
        fear not! We'll start from the very basics and guide you through each
        step along the way.
      </p>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Understanding the Keyboard</h3>
        <p>
          Before we begin typing, let's familiarize ourselves with the keyboard.
          Think of it as a canvas where your fingers will dance to bring your
          thoughts to life!
        </p>

        <div className="mt-4">
          <h4 className="mb-2 text-xl font-bold">Key Areas of the Keyboard:</h4>
          <ul className="list-disc pl-6">
            <li>
              <strong>Home Row:</strong> This is where your fingers naturally
              rest when you're not actively typing. For our right hand, it
              includes the keys J, K, L, and the semicolon (;).
            </li>
            <li>
              <strong>Home Position:</strong> The default resting position for
              your fingers when not typing, gently hovering over the Home Row
              keys.
            </li>
          </ul>
        </div>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Positioning Your Fingers</h3>
        <p>
          Just like a pianist positions their fingers on piano keys, we need to
          ensure our fingers are in the right position for efficient typing.
        </p>

        <ol className="list-decimal pl-6">
          <li>
            <strong>Sit Up Straight:</strong> Good posture is key! Sit
            comfortably with your back straight and your feet flat on the floor.
          </li>
          <li>
            <strong>Home Row Position:</strong> For our right hand, place your
            fingers on the keys J, K, and L.
            <ul className="mt-2 list-disc pl-6">
              <li>
                Your right index finger rests on <strong>J</strong>.
              </li>
              <li>
                Your right middle finger rests on <strong>K</strong>.
              </li>
              <li>
                Your right ring finger rests on <strong>L</strong>.
              </li>
            </ul>
          </li>
        </ol>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Typing Practice: JKL</h3>
        <p>
          Now, let's put theory into practice and start typing with our right
          hand!
        </p>

        <ol className="list-decimal pl-6">
          <li>
            <strong>Pressing "J":</strong>
            <ul className="mt-2 list-disc pl-6">
              <li>Keep your right index finger on the "J" key.</li>
              <li>
                Gently press down on the "J" key using your right index finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
          <li>
            <strong>Pressing "K":</strong>
            <ul className="mt-2 list-disc pl-6">
              <li>Move your right middle finger to the "K" key.</li>
              <li>
                Press down on the "K" key gently using your right middle finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
          <li>
            <strong>Pressing "L":</strong>
            <ul className="mt-2 list-disc pl-6">
              <li>Shift your right ring finger to the "L" key.</li>
              <li>
                Press down on the "L" key gently using your right ring finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
        </ol>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          The Benefits of Learning to Type
        </h3>
        <p>
          Learning to type isn't just about pressing keys; it's a skill that can
          unlock countless opportunities in both your personal and professional
          life.
        </p>

        <ul className="list-disc pl-6">
          <li>
            <strong>Personal Life:</strong> Typing efficiently allows you to
            communicate more effectively, whether it's chatting with friends,
            writing emails, or expressing your thoughts through text.
          </li>
          <li>
            <strong>Professional Life:</strong> In today's digital age, typing
            is a fundamental skill for many careers. From typing up reports to
            communicating with colleagues, efficient typing can boost your
            productivity and open doors to new job opportunities.
          </li>
        </ul>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          Understanding J, K, and L in the English Language
        </h3>
        <p>Let's take a moment to appreciate the characters we're typing:</p>

        <ul className="list-disc pl-6">
          <li>
            <strong>J, K, and L:</strong> These letters are essential components
            of the English language. From "jump" to "kite," "lake" to "love,"
            these characters play a significant role in our written
            communication.
          </li>
        </ul>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          Importance of Habitual Practice
        </h3>
        <p>
          Consistent practice is crucial to mastering typing skills and
          improving literacy. Just like learning any new skill, regular practice
          builds muscle memory and enhances proficiency over time. Remember,
          every keystroke brings you one step closer to mastery!
        </p>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Q/A Quiz Session</h3>
        <p>Now, let's assess your understanding with a quick quiz:</p>

        <ol className="list-decimal pl-6">
          <li>What are the Home Row keys for the right hand?</li>
          <li>Which finger should rest on the "K" key?</li>
          <li>What are two benefits of learning to type?</li>
          <li>
            Why is consistent practice important in improving typing skills?
          </li>
          <li>
            Can you provide an example of a word that contains the letter "L"?
          </li>
        </ol>
      </section>
    </div>
  );
}
