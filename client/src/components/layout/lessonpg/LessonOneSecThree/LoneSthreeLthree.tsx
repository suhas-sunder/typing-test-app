export default function LoneSthreeLthree() {
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      <h2 className="mb-4 text-3xl font-bold">
        Welcome to Your First Typing Lesson!
      </h2>
      <p>
        Today, we're going to dive into the fascinating world of keyboards and
        typing. If you've never typed before, don't worry; we'll start from the
        very beginning and guide you through each step of the way.
      </p>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Understanding the Keyboard</h3>
        <p>
          Let's start by getting familiar with the keyboard. Think of it as your
          canvas where your fingers will dance to bring your thoughts to life!
        </p>
        <ul className="list-disc pl-6">
          <li>
            <strong>Home Row:</strong> This is where your fingers naturally rest
            when you're not actively typing. For our right hand, it includes the
            keys A, F, and J.
          </li>
          <li>
            <strong>Home Position:</strong> The default resting position for
            your fingers when not typing, gently hovering over the Home Row
            keys.
          </li>
        </ul>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Positioning Your Fingers</h3>
        <p>
          Just like a pianist positions their fingers on the piano keys, we need
          to ensure our fingers are in the right position for efficient typing.
        </p>
        <ol className="list-decimal pl-6">
          <li>
            <strong>Sit Up Straight:</strong> Good posture is key! Sit
            comfortably with your back straight and your feet flat on the floor.
          </li>
          <li>
            <strong>Home Row Position:</strong> For our right hand, place your
            fingers on the keys A, F, and J.
            <ul className="list-disc pl-6">
              <li>
                Your right index finger rests on <strong>A</strong>.
              </li>
              <li>
                Your right middle finger rests on <strong>F</strong>.
              </li>
              <li>
                Your right ring finger rests on <strong>J</strong>.
              </li>
            </ul>
          </li>
        </ol>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          Typing Practice: A, F, and J
        </h3>
        <p>Now, let's practice typing with our right hand!</p>
        <ol className="list-decimal pl-6">
          <li>
            <strong>Pressing "A":</strong>
            <ul className="list-disc pl-6">
              <li>Keep your right index finger on the "A" key.</li>
              <li>
                Gently press down on the "A" key using your right index finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
          <li>
            <strong>Pressing "F":</strong>
            <ul className="list-disc pl-6">
              <li>Shift your right middle finger to the "F" key.</li>
              <li>
                Press down on the "F" key gently using your right middle finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
          <li>
            <strong>Pressing "J":</strong>
            <ul className="list-disc pl-6">
              <li>Move your right ring finger to the "J" key.</li>
              <li>
                Press down on the "J" key gently using your right ring finger.
              </li>
              <li>Release the key after pressing it.</li>
            </ul>
          </li>
        </ol>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          The Benefits of Learning to Type
        </h3>
        <ul className="list-disc pl-6">
          <li>
            <strong>Personal Life:</strong> Typing efficiently allows you to
            express yourself more effectively, whether it's chatting with
            friends, writing emails, or expressing your thoughts through text.
          </li>
          <li>
            <strong>Professional Life:</strong> In today's digital age, typing
            is a fundamental skill for many careers. From typing up reports to
            communicating with colleagues, efficient typing can boost your
            productivity and open doors to new job opportunities.
          </li>
        </ul>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          Understanding A, F, and J in the English Language
        </h3>
        <p>Let's take a moment to appreciate the characters we're typing:</p>
        <ul className="list-disc pl-6">
          <li>
            <strong>A:</strong> The letter "A" is frequently used in English
            words like "apple," "after," and "always."
          </li>
          <li>
            <strong>F:</strong> The letter "F" is commonly used in words like
            "fun," "friend," and "family."
          </li>
          <li>
            <strong>J:</strong> The letter "J" is found in words like "jump,"
            "joy," and "jungle."
          </li>
        </ul>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">
          Importance of Habitual Practice
        </h3>
        <p>
          Consistent practice is the key to mastering any skill, including
          typing. Just like learning to ride a bike, regular practice builds
          muscle memory and enhances proficiency over time. So, don't be
          discouraged by initial challenges; with dedication and practice,
          you'll become a typing expert in no time!
        </p>
      </section>

      <section className="mt-8">
        <h3 className="mb-4 text-2xl font-bold">Q/A Quiz Session</h3>
        <p>Now, let's assess your understanding with a quick quiz:</p>
        <ol className="list-decimal pl-6">
          <li>What fingers should be used to type the letter "A"?</li>
          <li>Which finger is responsible for typing the letter "F"?</li>
          <li>Can you name two benefits of learning to type?</li>
          <li>
            Why is consistent practice important in improving typing skills?
          </li>
          <li>Give an example of a word that contains the letter "J".</li>
        </ol>
      </section>
    </div>
  );
}
