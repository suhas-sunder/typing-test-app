...
export default function VerifyEmail() {
  ...
  const handleVerification = useCallback(async () => {
    ...
    if (
      !searchPath ||
      !searchPath.split("=")[1] ||
      searchPath.split("=")[0] !== "?emailToken"
    )
      navigate("/login"); //If pathname is invalid or empty, redirect to login page.

    const emailToken = searchPath.split("=")[1];
    ...
    <div className="absolute left-1/2 top-1/2 flex w-full max-w-[900px] -translate-x-1/2 -translate-y-1/2 flex-col items-center justify-center gap-8 text-center font-lato text-lg leading-loose tracking-widest text-slate-600">
      ...
      {!isVerified && !displayError && (
        <div>
          ...
          <p className="text-base leading-loose tracking-widest">
            If the verification is taking longer than 60 seconds with no success
            or error response, contact us at{" "}
            <span>
              <Link
      ...
      {isVerified && (
        <p>
          ...
          ! Your account <span className="font-nunito text-defaultblue">{accoutnDetails.email}</span> has been
      ...
      {displayError && (
        <ul className="font-nunito text-red-500 flex flex-col gap-5">
