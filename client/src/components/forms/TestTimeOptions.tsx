    ...
    <ul className="grid relative grid-cols-3 gap-y-8 justify-around w-11/12 text-xl my-8 sm:grid-cols-5 sm:gap-y-0 sm:justify-evenly sm:text-2xl">
      {timeOptions.map((time: string, index: number) => (
        <li key={index} className="flex justify-center items-center ">
          ...
          <label
            htmlFor={`radio-${time}`}
            className={`${styles["menu-label"]} flex z-[1] bg-white flex-col justify-center items-center h-20 w-20 border-2 border-slate-200 rounded-lg hover:text-sky-600 hover:cursor-pointer hover:border-sky-500 hover:font-medium sm:w-24 sm:h-24 outline-default-sky-blue"`}
          >
