...
export default async function CurriculumLessonPage({ params }: { params: Promise<LessonParams> }) {
  ...
  return (
    <PageFrame>
      <TypingTest title={`${lesson.sequence}. ${lesson.title}`} subtitle={`${lesson.objective}${fingers ? ` Finger guide: ${fingers}.` : ""}`} initialText={passage} testName={lesson.id} defaultDuration={120} defaultDifficulty="easy" lessonTargets={{ masteryWpm: lesson.masteryWpm, standardWpm: lesson.standardWpm }} lockText />
