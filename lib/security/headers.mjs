...
export const CANONICAL_PRODUCTION_HOSTNAME = "freetypingcamp.com";
export const STRICT_TRANSPORT_SECURITY_HEADER = Object.freeze({
...
export function buildSecurityHeaders({
  ...
  if (includeStrictTransportSecurity) {
    headers.push(STRICT_TRANSPORT_SECURITY_HEADER);
  }
  return headers;
}
...
export function shouldApplyStrictTransportSecurity({
  ...
  return (
    environment === "production" &&
    hostname?.toLowerCase() === CANONICAL_PRODUCTION_HOSTNAME
  );
}
...
export function buildRequestSecurityHeaders({
  ...
  return buildSecurityHeaders({
    advertising,
    environment,
    includeStrictTransportSecurity: shouldApplyStrictTransportSecurity({
      environment,
      hostname,
    }),
  });
