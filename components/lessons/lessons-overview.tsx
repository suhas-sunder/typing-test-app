...
export function LessonsOverview({
  ...
  const summary = useMemo(() => {
    const records = ENABLED_CURRICULUM_LESSONS.map(
      (lesson) => progress.lessons[lesson.id],
    );
    return {
      completed: records.filter((record) => record?.completed).length,
      next:
        ENABLED_CURRICULUM_LESSONS.find(
          (lesson) => !progress.lessons[lesson.id]?.completed,
        ) ?? null,
      stars: records.reduce(
        (total, record) => total + (record?.bestStars ?? 0),
        0,
      ),
    };
  }, [progress.lessons]);
      ...
      <div className="page-shell">
        <div className="max-w-3xl">
          ...
          <h1 className="heading-lg mt-2">
            Build accuracy one short lesson at a time.
          </h1>
          <p className="body-lg mt-3">
            Forty-five lessons move from beginner foundations through
            intermediate fluency and advanced application. Accuracy comes first;
            speed matters after the keystrokes are clean.
          </p>
        </div>
        ...
        <div className="mt-9 flex flex-wrap gap-x-10 gap-y-4 bg-camp-tan/45 px-5 py-5 sm:px-7">
          <SummaryMetric
            ...
            value={`${summary.completed}/45`}
          />
          <SummaryMetric
            label="Performance stars"
            value={`${summary.stars}/225`}
          />
          <div className="min-w-[14rem] flex-1">
            <div className="text-xs font-black uppercase tracking-[0.12em] text-camp-muted">
              Recommended next
            </div>
            {summary.next ? (
              <Link
              ...
              <p className="mt-1 font-black text-camp-sage">
        ...
        <div className="mt-14 grid gap-14">
          {CURRICULUM_LEVELS.map((level) => {
            ...
            const stars = lessons.reduce(
              (total, lesson) =>
                total + (progress.lessons[lesson.id]?.bestStars ?? 0),
              0,
            );
            return (
              <section
              ...
              >
                <div>
                  <p className="eyebrow">Level {level.sequence}</p>
                  <h2 id={`${level.id}-heading`} className="heading-md mt-2">
                    {level.title}
                  </h2>
                  <p className="mt-3 leading-7 text-camp-muted">
                    {level.summary}
                  </p>
                  <p className="mt-3 text-sm font-black text-camp-muted">
                    {completed}/{lessons.length} complete · {stars}/
                    {lessons.length * 5} stars
                  </p>
                </div>
        ...
        {beforeStarGuide ? (
          <div className="mt-16">{beforeStarGuide}</div>
        ) : null}

        <section
          className="mt-16 max-w-3xl"
          aria-labelledby="star-guide-heading"
        >
          <p className="eyebrow">Performance, not reviews</p>
          <h2 id="star-guide-heading" className="heading-md mt-2">
            How the five lesson stars work
          </h2>
          <p className="mt-3 leading-7 text-camp-muted">
            One star marks completion at 85% aggregate accuracy. Two and three
            stars raise the accuracy target to 90% and 95%. Four stars require
            97% accuracy plus the lesson&apos;s standard WPM; five require 99%
            accuracy plus its mastery WPM. Your best result never decreases.
          </p>
          <p className="mt-4 leading-7 text-camp-muted">
            Need more repetition without affecting lesson stars? Use{" "}
            <Link
              className="font-black text-camp-coral underline underline-offset-4"
              href="/typing-practice"
            >
              focused typing practice
            </Link>{" "}
            for keyboard rows, hands, quotes, numbers, and common words.
          </p>
        </section>
      </div>
    </section>
...
function SummaryMetric({ label, value }: { label: string; value: string }) {
  return (
    <div>
      <div className="text-2xl font-black text-camp-ink">{value}</div>
      <div className="text-xs font-black uppercase tracking-[0.12em] text-camp-muted">
        {label}
      </div>
    </div>
